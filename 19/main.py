...
def determine_new_rule(rule: list, transformed_rules: dict[str, list[str]]) -> list[list[str]]:
    while True:
        new_rule: list[list[str]] = []
...
def list_transform(lst: list[str], transformed_rules: dict[str, list[str]]) -> list[list[str]]:
...
def initialize_rules(rules_list: list[str]) -> tuple[dict[str, list[list[str]]], dict[str, list[str]]]:
    ...
    transformed_rules: dict[str, list[str]] = {}
...
def determine_rules(rules_list: list[str]) -> dict[str, list[str]]:
...
def valid_string(string: str, transformed_rules: dict[str, list[str]], version: int) -> bool:
...
def part_1_test(data: list[str]) -> int:
...
def constructor(data: list[str], version: int) -> int:
    rules_list = data[0].splitlines()
    transformed_rules = determine_rules(rules_list)
    strings = data[1].splitlines()
